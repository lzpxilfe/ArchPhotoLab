...
- 도면을 정합 모드별로 warp하여 overlay 생성
- 점 개수 부족/대응점 모드 실패 시 즉시 오류 메시지 표시
...
- 평균/중앙값/최대 오차 + 품질등급 표시
- 오차 큰 점은 색상 경고와 함께 상태에 반영

### 7) 플랫 보정
- 배경 조명 추정 + CLAHE + 감마 기반 1차 보정
- `기록용`, `강한음영완화`, `부드러운정리` 프리셋과 강도 슬라이더 지원
- `원본/평탄화 보기`, `분할 비교`로 판독성 검증
...
- `format_version` 업그레이드 및 누락 필드 보정 지원
...
- 실행 시 스플래시 화면에서 `icon.png`와 공개형 라이선스 안내
...
- 정합은 `homography`, `affine`, `similarity` 제공(RANSAC/추가 자동점추출은 추후 확장)
- 자동 대응점 추출은 미구현
- 한 번에 1장의 사진, 1장의 도면 기준

## 🧪 샘플/QA 자원

- `samples/README.md`: 시나리오 기반 샘플 프로젝트 템플릿 사용법
