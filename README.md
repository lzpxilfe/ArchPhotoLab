...
아래는 실제 도면과 드론 사진을 정합한 예시입니다.

### 입력 이미지 1 — 드론 항공사진
![드론 항공사진 예시](docs/images/sample_drone_photo.jpg)

> 드론으로 촬영한 고해상도 수직/사선 항공사진. 도로, 밭두렁, 건물 등 식별 가능한 지형지물이 있어야 합니다.

### 입력 이미지 2 — 도면 (유적 범위 및 지형도)
![도면 예시](docs/images/sample_plan_map.jpg)

> 고고학 지표조사 도면. 등고선, 유적 범위(빨간 외곽선), 경지 구획 등이 표현되어 있습니다.

### 출력 결과 — 도면 오버레이 합성
![정합 결과 예시](docs/images/sample_overlay_result.jpg)

> 두 이미지가 정합된 후 도면이 사진 위에 정확히 겹쳐진 결과입니다. TPS 자유 변형 방식을 사용하면 구부러진 등고선도 지형에 맞게 자연스럽게 보정됩니다.
